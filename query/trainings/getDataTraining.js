  ...
  try {
    ...
    if (trainings_id !== undefined && trainings_id !== null) {
      if (!Array.isArray(trainings_id)) {
        validationErrors.push("trainings_id");
      } else {
        // التحقق من أن جميع العناصر في المصفوفة أرقام صحيحة
        for (let i = 0; i < trainings_id.length; i++) {
          if (trainings_id[i] !== 0 && (isNaN(trainings_id[i]) || trainings_id[i] <= 0)) {
            validationErrors.push("trainings_id");
            break;
          }
        }
    ...
    // إذا كانت هناك أخطاء في التحقق، قم بإرجاع جميع التدريبات مع اسم الحقل الخاطئ
    if (validationErrors.length > 0) {
      const allTrainingsResult = await getAllData("trainings");
