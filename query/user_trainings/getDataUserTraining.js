...
const getDataUserTrainingsByUserId = async (req, res) => {
  try {
    ...
    if (result.status === "success") {
      ...
      const processedData = result.data.map(training => {
        if (training.equipment_weights) {
          try {
            // Try to parse as JSON
            training.equipment_weights_parsed = JSON.parse(training.equipment_weights);
          } catch (e) {
            // If not valid JSON, keep as string
            training.equipment_weights_parsed = training.equipment_weights;
          }
        }
        return training;
      });
